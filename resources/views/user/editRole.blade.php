            ...
            <div class="col-md-12">
                ...
                <div class="panel no-border">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="row">
                            @foreach($permission_group as $permission_item)
                            <?php $status = ($permission_role->contains('permission_id', $permission_item->id) ? 'checked="checked"' : '') ?>
                            <div class="col-xs-4">
                                <div class="checkbox checkbox-theme">
                                    <input type="checkbox" name="permissions[]" id="permission_{{$permission_item->id}}" value="{{$permission_item->id}}" {{ $status }}>
                                    <label for="permission_{{$permission_item->id}}">{{ $permission_item->display_name }}</label>
                                </div>
                            </div>
                            @endforeach
                        </div>

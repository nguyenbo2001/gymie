...
class SubscriptionsController extends Controller
{
    ...
    public function index(Request $request) {
        $subscriptions = Subscription::indexQuery($request->sort_field,
                                    ...
                                    ->paginate(10);
        $subscriptionTotal = Subscription::indexQuery($request->sort_field,
                                                $request->sort_direction,
                                                $request->drp_start,
                                                $request->drp_end,
                                                $request->plan_name)
    ...
    public function expiring(Request $request) {
        $expirings = Subscription::expiring($request->sort_field,
                                            $request->sort_direction,
                                            $request->drp_start,
                                            $request->drp_end)
                                ->search('"'. $request->input('search').'"')
                                ->paginate(10);
        $expiringTotal = Subscription::expiring($request->sort_field,
                                            $request->sort_direction,
                                            $request->drp_start,
                                            $request->drp_end)
                                ->search('"'. $request->input('search').'"')
    ...
    public function expired(Request $request) {
        $allExpired = Subscription::expired($request->sort_field,
                                            $request->sort_direction,
                                            $request->drp_start,
                                            $request->drp_end)
                                ->search('"'. $request->input('search').'"')
                                ->paginate(10);
        $allExpiredTotal = Subscription::expired($request->sort_field,
                                            $request->sort_direction,
                                            $request->drp_start,
                                            $request->drp_end)
                                ->search('"'. $request->input('search').'"')
    ...
    public function renew(Request $request, $id) {
        ...
        $subscription = Subscription::where('invoice_id', $id)->get();
    ...
    public function modify(Request $request, $id) {
        ...
        try {
            ...
            // Helper function to set Payment Status
            $invoice_total = $request->admission_amount +
                            $request->subscriptioni_amount +
                            $request->taxes_amount -
                            $request->discount_amount;
            $paymentStatus = \constPaymentStatus::Unpaid;
            $total_paid = $request->payment_amount +
                        $request->previous_payment;
            ...
            Invoice::where('id', $subscription->invoice_id)
                    ->update([
                        'invoice_number' => $request->invoice_number,
                        'total' => $invoice_total,
                        'status' => $paymentStatus,
            ...
            foreach ($request->plan as $plan) {
                ...
                // Adding subscription to invoice (Invoice Detail)
                InvoiceDetail::where('invoice_id', $subscription->invoice_id)
                            ->update([
                                'plan_id' => $plan['id'],
                                'item_amount' => $plan['price'],
                            ]);

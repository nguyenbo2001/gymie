...
class DataMigrationController extends Controller
{
    ...
    public function migrate() {
        DB::beginTransaction();
        try {
            // From invoice detail > tax to invoice tax
            $oldInvoiceDetailsTax = InvoiceDetail::where('item_name', '=', 'Taxes')->get();
            foreach ($oldInvoiceDetailsTax as $taxDetail) {
                Invoice::where('id', $taxDetail->invoice_id)
                        ->update(['tax' => $taxDetail->item_count]);
            }
            ...
            foreach ($oldInvoiceDetailsAdmission as $admissionDetail) {
                Invoice::where('id', $admissionDetail->invoice_id)
                        ->update(['additional_fees' => $admissionDetail->item_count]);
            }

            // Now delete the admission and tax rows from Invoice Detail
            ...
            foreach ($invoices as $invoice) {
                InvoiceDetail::where('invoice_id', $invoice->id)
                            ->update(['plan_id' => $invoice->member->plan_id]);
            }
    ...
    public function migrateExcel() {
        ...
        try {
            foreach ($lines as $line) {
                ...
                $start_date = Carbon::createdFromFormat('d/m/Y', $line->get('start_date'));
                ...
                Setting::where('key', '=', 'invoice_last_number')
                        ->update(['value' => $invoiceCounter]);

                Setting::where('key', '=', 'member_last_number')
                        ->update(['value' => $memberCounter]);
            }
            DB::commit();
        } catch(\Exception $e) {
            DB::rollback();

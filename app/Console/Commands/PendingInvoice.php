...
class PendingInvoice extends Command
{
    ...
    {
        $invoices = Invoice::select(DB::raw('*, DATE_ADD(created_at, INTERVAL 4 DAY) AS sms_start, DATE_ADD(created_at, INTERVEL 6 DAY) AS sms_end'))
                            ->havingRaw('CURDATE() >= sms_start AND CURDATE() <= sms_end')
                            ->whereIn('status', [0, 2])
                            ->get();
        $sms_trigger = SmsTrigger('alias', '=', 'pending_invoice')->first();
